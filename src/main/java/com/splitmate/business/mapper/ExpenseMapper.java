...
public interface ExpenseMapper
{
    ...
    ExpenseEntity mapDTOToEntity(CreateExpenseDTO dto, GroupEntity groupEntity, UserEntity paidBy);

    @Mapping(source = "groupEntity.id", target = "groupId")
    @Mapping(source = "paidOutBy.id", target = "paidBy")
    ExpenseInfoDTO mapEntityToDTO(ExpenseEntity expenseEntity);

    @Mapping(target = "paidOutBy", source = "paidBy")
    @Mapping(target = "splits", ignore = true)
    void mapUpdateDTOToEntity(UpdateExpenseDTO dto, UserEntity paidBy, @MappingTarget ExpenseEntity expenseEntity);
}
